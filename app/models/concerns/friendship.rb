module Friendship
  ...
  included do
    ...
    has_and_belongs_to_many :inverse_friends,
  ...
  def make_friend(user)
    return false if user == self || find_any_relationship_with(user) || is_subscrition_blocked?(user)
    Relationship.new(friend_id: user.id, user_id: self.id).save
  end
